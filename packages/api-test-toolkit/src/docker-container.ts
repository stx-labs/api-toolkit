...
async function ensureContainerRunning(docker: Docker, config: DockerTestContainerConfig) {
...
export async function dockerTestUp(args: { config: DockerTestContainerConfig }): Promise<void> {
  const { config } = args;
  const host = config.host ?? DEFAULTS.host;
...
export async function dockerTestDown(args: { config: DockerTestContainerConfig }): Promise<void> {
  const { config } = args;
  const docker = createDockerClient();
...
export async function dockerTestLogs(args: {
